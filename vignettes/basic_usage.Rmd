...
Alternatively you can use the `plot_prediction()` method.
...
abc$plot_posterior(sample = 735, prior = TRUE, uncertainty_type = "conformal") +
...
# Print a sample with -5 < x1 < -4 (within the distribution with a low noise)
# which(abc$observed$X1 < 2 & abc$observed > -2)
abc$plot_posterior(sample = 501, prior = TRUE, uncertainty_type = "uncertainty") +
  geom_vline(xintercept = Y_obs[501], color = "red", size = 1.5)
Y_obs[501]
abc$predictive_mean$y1[501]
...
# Print a sample out of distribution
# which(abc$observed$X1 > 7)
abc$plot_posterior(sample = 735, prior = TRUE, uncertainty_type = "uncertainty") +
  geom_vline(xintercept = Y_obs[735], color = "red", size = 1.5)
Y_obs[735]
abc$predictive_mean$y1[735]
```
...
```{r}
expl = explain$new(abc, method = "deeplift")

expl$run(observed)

# Use the method on the test data
expl$plot()
