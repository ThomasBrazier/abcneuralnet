...
The `abcnn` object is a `R6Class` object, providing a modern Object-Oriented Programming (OOP) approach. All the settings and data of an `abcnn` object can be accessed in public slots. For example, the `torch` model can be accessed with `abcnn$model`, and the `luz` fitted model can be accessed with `abcnn$fitted`.
...
```{r, echo = T, fig.height = 4, fig.width = 8, fig.align="center", fig.cap="The training curve of the neural network across 30 epochs, computed on training data split in two partitions: training and validation. The black horizontal line is the loss computed on the evaluation dataset at the end of the training procedure.}
...
```{r, echo = T, fig.height = 4, fig.width = 8, fig.align="center", fig.cap="Predictions as a function of simulated parameter. The purple ribbon is the 95% Conformal Credible Interval based on the epistemic unvertainty alone. The gree ribbon is the 95% Conformal Credible Interval based on the overall unvertainty."}
...
```{r, echo = T, fig.height = 4, fig.width = 8, fig.align="center, fig.cap="Predictions as a function of simulated parameter. The epistemic uncertainty (red) and aleatoric uncertainty (blue) were estimated with Concrete Dropout (as in Gal et 2016).}
ggplot(data = df_training, aes(x = x, y = y)) +
  geom_point(color = "blue", alpha = 0.3) +
  # geom_point(data = df_predicted, aes(x = x, y = y_true), color = "green", alpha = 0.3) +
  geom_line(data = df_predicted, aes(x = x, y = predictive_mean), color = "Red") +
