...
Throughout this tutorial, I will use the three core methods developed in `ABCNeuralNet`, sepcifically Concrete Dropout, Deep Ensemble and Tabnet-ABC. I will use simple toy models to show how to initialize an `abcnn` object, then fit, predict on test and observed samples, evaluate the performance of the trained neural network and of the predictions on observed data.


ABCNeuralNet uses the `torch` R package, which is a very efficient implementation leveraging the Torch C++ library, and providing codes and functions very similar to the PyTorch framework. Neural networks are fitted with the `luz` R package. If you are beginners with `torch` in R and want to know more, I strongly encourage you to read the book ['Deep Learning and Scientific Computing with R torch'](https://skeydan.github.io/Deep-Learning-and-Scientific-Computing-with-R-torch/) by Sigrid Keydana, in open access.




The data analysis workflow in ABCNeuralNet is simple and user-friendly, and generally follow these sequential steps:


(1) Initialize and `abcnn` `R6` object with the `abcnn$new()` method. At this step you provide the training samples (variables and parameters to infer), and the observed variables for which you wish to infer the parameter. In the ABC framework, the variables are the summary statistics. You also set the hyperparameters of the neural network at this step (e.g. number of layers, neurons, epochs, the learning rate).
...
(4) Predict on test samples with `abcnn$predict()`. If necessary, the test samples can be changed at this step with `abcnn$predict(data = new_data)`, so that you can predict many different datasets from the same trained neural network.

(5) Assess the goodness of fit on test samples with the graphical outputs of `abcnn$plot_prediction()` and `abcnn$plot_posterior()`.

(6) Predict on observed samples with `abcnn$predict(data = observed_data)` and look at your results with `abcnn$plot_posterior()` for a figure and `abcnn$predictions()` for the table of your estimates.





### Begin with a simple linear regression problem


For this first example, I simulated 2,000 training samples and 1,000 test samples with a simple linear function. There is only one parameter to estimate, and one input variable.
...
We use a Concrete Dropout regression model described in [ref]. You can read an intesresting discussion on the Concrete Dropout model on this [blog post](https://blogs.rstudio.com/ai/posts/2018-11-12-uncertainty_estimates_dropout/) for more details.


First, I initialize the `abcnn` object, with the training samples `theta` (parameters of the simulations), `sumstats` (the summary statistics) and `observed`, the summary statistics of the test samples simulated. As it is a simulated dataset, I do not have observed data to predict at the end. At this step you define all the hyperparameters of your neural network.
...
The `abcnn` object is a `R6Class` object, providing a modern Object-Oriented Programming (OOP) approach. All the settings and data of an `abcnn` object can be accessed in public slots. For example, the `torch` model can be accessed with `abcnn$fitted$model`, and the `luz` fitted model can be accessed with `abcnn$fitted`.
...
Plotting the training curve is a really common procedure during the training of a neural network. It gives you a lot of informations on the quality of the training, especially to assess over- and under-fitting of the network. Training a neural network usually involves a trial-error process with fine-tuning of the hyperparameters until a good performance is achieved. This is why there is an `abcnn$plot_training()` method to rapidly plot it in a single command line.
...
Note that sometimes the heteroscedastic loss value can be negative with the Concrete Dropout method, and it can be confusing as MSE losses generally used are always positive. However, it does not impact the training procedure.


### Predicting parameters with the Concrete Dropout method


After the model is efficiently trained and the training quality has been assessed, parameter estimates can be predicted with `abc$predict()`. In this case I predict on 1,000 simualted samples.
...
You can get the neural network's predictions as a tidy data frame.
...
You can use `abcnn$predictions()` to get the estimates with their associated credibility intervals.
...
```{r, echo = T, fig.height = 4, fig.width = 8, fig.align="center", fig.cap="Predictions as a function of simulated parameter. The purple ribbon is the Conformal Credible Interval based on the epistemic unvertainty alone. The gree ribbon is the Conformal Credible Interval based on the overall unvertainty."}
...
```{r, echo = T, fig.height = 4, fig.width = 8, fig.align="center", fig.cap="Predictions as a function of simulated parameters. The epistemic uncertainty (red) and aleatoric uncertainty (blue) were estimated with Concrete Dropout (Gal et 2016)."}
ggplot(data = df_training, aes(x = x, y = y)) +
  geom_point(color = "blue", alpha = 0.3) +
  # geom_point(data = df_predicted, aes(x = x, y = y_true), color = "green", alpha = 0.3) +
  geom_line(data = df_predicted, aes(x = x, y = predictive_mean), color = "Red") +
...
Alternatively you can directly use `plot_prediction()` and `abcnn$plot_posterior()` to plot predictions in two different ways, either if you are interested in all samples at a time, or just a single sample. You can choose the uncertainty plotted, either "conformal" for Conformal Credible Invertvals (default), "uncertainty" for uncertainty direct estimates, and "posterior quantile" for the approximate posterior distribution (except for Deep Ensemble). 
...
To focus on a single prediction, you can look at the distribution of approximate posterior estimates with predictive means and credible intervals. The prior distribution is plotted underneath (white bars) to compare priors and posteriors.
...
ggplot(df, aes(x = true.theta, y = predictive_mean)) +
  geom_errorbar(aes(ymin = posterior_lower_ci, ymax = posterior_upper_ci), fill = "lightgrey", alpha = 0.1) +
  geom_point() +
...
## The `explainn()` class and methods


[https://bips-hb.github.io/innsight/](innsight R package)
...
2. Validation. During training, `luz` evaluates the performance of the model on a separate validation dataset on this validation set every epoch to monitor overfitting and generalization.



3. Testing. After training, evaluate the trained model on an unseen test set. With ABCNeuralNet, you can leverage the multi-output feature and pass a test set (a simulated dataset with known truth) along with the observed data. Use `abcnn$predict()` method to get predictions on both the test set and the observed set, and calculate metrics on the test set.



4. Diagnostics. Check training and validation loss curves with `abcnn$plot_training()` to monitor under/overfitting. Check the relationship between predicted and true values on the test set. This flexibility of passing a test set directly to the `abcnn$predict()` method allows to optionally implement custom metrics for deeper insight.
...
It is important that you feel free to contact me for any questions and comments on your usage of the package (post your issues). I will do my best to document any troubleshooting encountered and to address bugs. This package is under continuous development and improvements.
...
It also means that if your training set (simulations) is not informative or your model is not appropriate to the observed data, the neural network will explicitly tell you that he is not able to learn: the learning will fail, with a loss function weakly decreasing across epochs, or a huge stochastic variation in loss values among epochs (that is why checking the training curve is crucial). Even if learning succeeds, a poorly specified model or non-informative simulations would only produce drastically large uncertainty values and Credible Intervals.
...
*7. How do I choose the hyperparameters of the model?*
