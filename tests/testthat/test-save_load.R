test_that("save_abcnn() and load_abcnn() functions work", {
  ...
  for (m in methods) {
    ...
    # Write to the session temporary directory, as `test-integration.R` does.
    # A package must not write inside its own directory when it is checked, and
    # the saved model is regenerated by this test anyway.
    temp_prefix = tempfile(paste0(gsub(" ", "_", m), "_saveload_"))

    save_abcnn(abc, prefix = temp_prefix)

    # Is it possible to fit and predict again?
    abc = load_abcnn(prefix = temp_prefix)
